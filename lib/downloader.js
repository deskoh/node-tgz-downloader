...
function downloadFromIterable(tarballsIterable, options) {
  ...
  return _downloadTarballs(tarballs, options.directory, options.mirror);
}
...
function _downloadTarballs(tarballs, baseDirectory = './tarballs', mirror = false) {
  ...
  const promises = tarballs.map(({ url, directory }, i, arr) => {
    ...
    return _downloadFileWithRetry(url, join(baseDirectory, directory, mirror ? '-' : ''), position, 10);
  });
